...
export function Wallet({ onBack }: WalletProps) {
	...
	const totalBalance = 1245.5;
	const savingsBalance = 850.0;
	const savingsGoal = 1000.0;
							...
							<div className="grid grid-cols-2 md:grid-cols-3 gap-4">
								{badges.map((badge) => (
									...
									>
										...
										<p
											className={badge.earned ? "text-white" : "text-[#7D8B91]"}
										>
											{badge.name}
										</p>
									</div>
									...
									>
										...
										<div
											className={`w-20 h-20 mx-auto mb-3 rounded-full flex items-center justify-center ${
												avatar.unlocked
													? "bg-white/20"
													: "bg-white border-2 border-[#E8EFF2]"
											}`}
										>
											<div
												className="w-12 h-12 rounded-full"
												style={{
													backgroundColor: avatar.unlocked
														? "white"
														: avatar.color,
